...
count_er <- count_model$variable.importance
count_er <- data.frame(predictor = names(count_er), importance = unname(count_er)) %>% 
  arrange(desc(importance))
# plot top 10 predictors
ggplot(head(count_er, 10)) + 
  aes(x = fct_reorder(predictor, importance), y = importance) +
  geom_col() +
  geom_hline(yintercept = 0, linewidth = 2, colour = "#555555") +
  scale_y_continuous(expand = c(0, 0)) +
