...
For the final filtering step, we'll use spatial boundaries from Australia's bioregions to subset the data to the region around Brisbane.

```{r}
#| label: abundance-data-region
study_region <- read_sf("data/ibra61_reg.gpkg") %>%
  filter(REG_CODE == "SEQ" & STATE == "QLD") %>%
...
<!--TODO ebirdst_predictors is old-->
...
pred_grid_all <- read_parquet("data/prediction-grid_year_AU-QLD_2022.parquet")
...
Based on the above plot, it appears that about 5 hours before solar noon is the optimal time for observing the species. Let's add all the effort variables to the prediction grid.
